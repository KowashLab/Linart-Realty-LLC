...
export default function SeedPage() {
  ...
  const handleSeedAll = async () => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/server/make-server-dcec270f/seed-all`,
        {
          method: 'GET',
          headers: {
  ...
  const handleSeedBlog = async () => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/server/make-server-dcec270f/blog/seed`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
            ...
            <div className="space-y-6">
              ...
              <details className="pt-4">
                ...
                <div className="mt-3 p-3 bg-[#0A0A0B] border border-[#E5E4E2]/10 font-mono text-xs text-[#E5E4E2]/50 break-all space-y-1">
                  ...
                  <div>Endpoint: <span className="text-[#E5E4E2]/70">https://{projectId}.supabase.co/functions/v1/server/make-server-dcec270f/seed-all</span></div>
                </div>
              </details>
            </div>
          </div>
