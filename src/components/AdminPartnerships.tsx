...
export function AdminPartnerships() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId
        ? `https://${projectId}.supabase.co/functions/v1/make-server-dcec270f/partnerships/admin/${editingId}`
        : `https://${projectId}.supabase.co/functions/v1/make-server-dcec270f/partnerships/admin`;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-dcec270f/partnerships/admin/${id}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
